...
WiFiClient espClient;           // Crea un client che puÃ² connettersi a un indirizzo IP
...
// Usiamo long perchÃ© il valore di millis() cresce molto rapidamente.
long lastMsg = 0;
 ...
 */
void setup() {
  // Inizializza la comunicazione seriale a 115200 baud (velocitÃ  bit/secondo)
  Serial.begin(115200);

  tft.begin();
  tft.setRotation(3); // Ruota lo schermo in modalitÃ  landscape (orizzontale)
...
void setup_wifi() {
  ...
  // Rimane in questo ciclo finchÃ© lo stato non diventa WL_CONNECTED
 ...
 */
void reconnect() {
  // Ciclo finchÃ© non siamo di nuovo online sul broker
